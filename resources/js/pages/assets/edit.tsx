...
export default function Edit({ asset }: { asset: Asset }) {
    ...
    const categories = (page.props.categories || []) as CategoryOption[];
    const tags = (page.props.tags || []) as TagOption[];
    const locations = (page.props.locations || []) as LocationOption[];
    ...
    const { data, setData, patch, processing, errors } = useForm<{
        ...
        tags: asset.tags ? asset.tags.map(String) : [],
    ...
    useEffect(() => {
        ...
        if (!flash?.createdCategory) return;
        if (handledCreatedCategoryId.current === flash.createdCategory.id) return;
    ...
    }, [page.props.flash, setData]);

    useEffect(() => {
        if (pendingDialog !== 'category' || isCategorySelectOpen) return;
    ...
    useEffect(() => {
        const flash = page.props.flash as PageProps['flash'];
        if (!flash?.createdTag) return;
        if (handledCreatedTagId.current === flash.createdTag.id) return;
    ...
    useEffect(() => {
        if (pendingDialog !== 'tag' || isTagSelectOpen) return;
        const timeoutId = window.setTimeout(() => {
            setPendingDialog(null);
    ...
    useEffect(() => {
        const flash = page.props.flash as PageProps['flash'];
        if (!flash?.createdLocation) return;
        if (handledCreatedLocationId.current === flash.createdLocation.id) return;
    ...
    useEffect(() => {
        if (pendingDialog !== 'location' || isLocationSelectOpen) return;
        const timeoutId = window.setTimeout(() => {
            setPendingDialog(null);
    ...
    return (
        <div className="w-full">
            ...
            <div className="w-full border-b px-6 py-4 mb-6">
                <h1 className="text-2xl font-bold tracking-tight">{data.name || 'Untitled Asset'}</h1>
            </div>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                            ...
                            <div className="md:col-span-2">
                                <Input id="name" value={data.name} onChange={e => setData('name', e.target.value)} className="rounded" placeholder="e.g. MacBook Pro" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                            ...
                            <div className="md:col-span-2">
                                <div className="flex -space-x-px">
                                    <div className="flex items-center rounded-l border border-input bg-muted px-3 text-sm text-muted-foreground">USD</div>
                                    <Input id="value" value={data.value} onChange={e => setData('value', e.target.value)} className="rounded-l-none rounded-r focus-visible:z-10" placeholder="0.00" />
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                            ...
                            <div className="md:col-span-2">
                                <Input id="description" value={data.description} onChange={e => setData('description', e.target.value)} className="rounded" placeholder="Add a short description" />
            ...
            <CategoryFormDialog open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen} redirectTo={`/assets/${asset.id}/edit`} preserveState={true} />

            <TagFormDialog open={isTagDialogOpen} onOpenChange={setIsTagDialogOpen} redirectTo={`/assets/${asset.id}/edit`} preserveState={true} />

            <LocationFormDialog open={isLocationDialogOpen} onOpenChange={setIsLocationDialogOpen} parentOptions={locations} redirectTo={`/assets/${asset.id}/edit`} preserveState={true} />
        </div>
    );
}
